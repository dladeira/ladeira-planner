...
export function isInYear(year, week, dayIndex) {
    ...
    if ((week == 0 && dayIndex < firstWeekDay) || (week == getWeeksInYear(year) && dayIndex > lastWeekDay))
        return false
    return true
}
...
export function getDateText(day, week, year) {
    var days
    if (daysInFirstWeek(year) == 7 && week == 1) {
        days = day
    } else {
        day = day + 1
        days = (week - 1) * 7 + day + (daysInFirstWeek(year) == 7 ? 0 : daysInFirstWeek(year))
    }
    var date = new Date(((year - 1970) * 31556952000) + (days * 24 * 60 * 60 * 1000))
    return String(date.getDate()).padStart(2, '0') + "." + String(date.getMonth() + 1).padStart(2, '0')
}

function daysInFirstWeek(year) {
    var days = 0
    for (var i = 0; i < 7; i++) {
        if (isInYear(year, 0, i)) {
            days++
        }
    }
